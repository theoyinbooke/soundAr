...
pub fn dynamic_tools() -> Value {
    let mut tools = json!([
        { "name": "get_studio_state", "description": "Inspect soundAr models, voices, projects, generation history, jobs, and scheduler state before planning audio work. 'BreezeBlue/Breeze-TTS-2' is the default voice model for new speech unless the request needs cloning, a preset voice, or a language it does not cover.", "inputSchema": { "type": "object", "properties": {} } },
        ...
        { "name": "queue_speech_generation", "description": "Queue local text-to-speech generation in soundAr and return a durable job id. Default to model_id 'BreezeBlue/Breeze-TTS-2' with voice_id '__engine_default__' whenever the user has not named a model or picked a library voice: Breeze is the most expressive installed voice. Choose a different model only when the request needs something Breeze cannot do - cloning a reference voice, using a named preset voice, a language outside English or Chinese, or when Breeze is not installed. Requires Studio or Full access.", "inputSchema": { "type": "object", "required": ["text", "model_id", "voice_id"], "properties": { "text":{"type":"string"}, "model_id":{"type":"string"}, "voice_id":{"type":"string"}, "title":{"type":"string"}, "language":{"type":"string"}, "priority":{"type":"string","enum":["low","normal","high"]} } } },
        { "name": "queue_speech_batch", "description": "Queue an ordered batch of local speech generations for a campaign, course, audiobook, podcast, or other multi-part plan. Rows without a model_id default to 'BreezeBlue/Breeze-TTS-2', the most expressive installed voice; name a different model per row only when the row needs cloning, a preset voice, or a language Breeze does not cover. Requires Studio or Full access.", "inputSchema": { "type":"object", "required":["rows"], "properties": { "rows":{"type":"array","minItems":1,"maxItems":500,"items":{"type":"object","required":["text"],"properties":{"name":{"type":"string"},"text":{"type":"string"},"model_id":{"type":"string"},"voice_id":{"type":"string"},"priority":{"type":"string","enum":["low","normal","high"]},"settings":{"type":"object"}}}}, "parallelism":{"type":"integer","minimum":1,"maximum":8} } } },
...
mod tests {
    ...
    fn dynamic_tool_catalog_supports_access_changes_without_resetting_the_thread() {
        ...
        assert!(names.contains(&"ensure_episode_cover"));
        assert!(names.contains(&"generate_episode_clips"));
        assert!(names.contains(&"edit_video_timeline"));
        assert!(names.contains(&"write_video_script"));
        assert!(names.contains(&"generate_cue_music"));
        assert!(names.contains(&"narrate_turns"));
        ...
        assert!(names.contains(&"transcribe_and_check_episode"));
