...
for executable in \
  "$inspection_root/deb/usr/bin/soundar-desktop"; do
...
magic="$(od -An -tx1 -N4 "$inspection_root/deb/usr/bin/soundar-desktop" | tr -d ' \n')"
[[ "$magic" == "7f454c46" ]] || {
  printf 'The packaged Debian binary is not an ELF executable.\n' >&2
  exit 1
}
...
if [[ "$ALLOW_UNSIGNED" == "1" ]]; then
  printf 'Verified unsigned local soundAr %s Debian runtime resources and playback policy.\n' "$VERSION"
else
  printf 'Verified soundAr %s Debian package, signatures, runtime resources, and playback policy.\n' "$VERSION"
fi
